...
?>
  ...
  <head>
    ...
    <link rel="stylesheet" href="TutorHome.css" />
...
900;1,100..900&display=swap" rel="stylesheet" />
  ...
  <body>
    ...
    <div class="container info">
      ...
      <h5>
        <?php
        echo "<img src='$language.svg' width='30' height='30' />          ";
    ...
    <div class="container">
      <div class="container right">
        <div class="sessions">
          ...
          <?php
          $query = "SELECT * FROM StudentSession WHERE tutor='$TutorName' AND status='accepted' AND completed=0 ";
          ...
          if (mysqli_num_rows($result) > 0) {
            while ($row = mysqli_fetch_assoc($result)) {
              ...
              echo "<p style='font-weight: bold;'>{$row['language']} Session with {$row['student']}</p>";
              echo "<p>Sunday{$row['day']}</p>";
              echo "<p>Duration: {$row['duration']}</p>";
              echo "<p>Perferred Slot: {$row['time']}</p>";
              // echo "<a style='text-decoration:none'; href='Request.php?id=$requestID' class='btn'>View Details</a>";
              echo "</div>";
              // echo "<div class='allRequests'> <a href='Requests.html'>View all requests</a> </div>";
            }
          } else {
        ...
        <div class="sessions">
          ...
          <?php
          $query = "SELECT * FROM StudentSession WHERE tutor='$TutorName' AND completed=1";
          $result = mysqli_query($conn, $query);
          ...
          if (mysqli_num_rows($result) > 0) {
            while ($row = mysqli_fetch_assoc($result)) {
              $requestID = $row['id'];
              echo "<div class='request'>";
              echo "<p style='font-weight: bold;'>{$row['language']} Session</p>";
              echo "<p>Sunday{$row['day']}</p>";
              echo "<p>Duration: {$row['duration']}</p>";
              echo "<p>Perferred Slot: {$row['time']}</p>";
              //echo "<a style='text-decoration:none'; href='Request.php?id=$requestID' class='btn'>View Details</a>";
        ...
        <div class="requests">
          <h3>New Requests</h3>
          <?php
          $query = "SELECT * FROM StudentSession WHERE tutor='$TutorName' AND status='pending'";
          $result = mysqli_query($conn, $query);
          ...
          if (mysqli_num_rows($result) > 0) {
            while ($row = mysqli_fetch_assoc($result)) {
              ...
              echo "<p style='font-weight: bold;'>{$row['language']} Session</p>";
              echo "<p>Sunday{$row['day']}</p>";
              echo "<p>Duration: {$row['duration']}  <a style='text-decoration:none; display:inline; margin-left:30px; color:#fff' href='Request.php?id=$requestID' class='btn'>View Details</a> </p>";
              echo "<p>Perferred Slot: {$row['time']}</p>";

              echo "</div>";
              // echo "<a style='text-decoration:none; display:inline-block'; href='Request.php?id=$requestID' class='btn'>View Details</a>";
            }
          } else {
            echo "<p>No upcoming sessions.</p>";
          }
          ?>
          <div class='allRequests'> <a href='Requests.php'>View all requests</a> </div>
        </div>
      </div>
      ...
      <div class="container left">
        <div class="students">
          <h3>Students</h3>
          <?php
          $query = "SELECT * FROM StudentSession WHERE tutor='$TutorName' AND status='accepted'";
          $result = mysqli_query($conn, $query);
          ...
          if (mysqli_num_rows($result) > 0) {
            while ($row = mysqli_fetch_assoc($result)) {
              ...
              echo "<p style='font-weight: bold;'>{$row['student']}</p>";
            ...
            echo "<p>No registered students. </p>";
          } 
          ?>
        </div>
        ...
        <div class="reviews">
          ...
          <?php
          $query = "SELECT * FROM reviews WHERE tutor_name='$TutorName'";
          $result = mysqli_query($conn, $query);
          ...
          echo "<p><strong>$student_name</strong></p>";
