...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  // $avatar = $_POST["avatar"];
    ...
    $result = $conn->query("SELECT MAX(tutor_id) as max_id FROM users WHERE role = 'tutor'");
    $row = $result->fetch_assoc();
    $tutor_id = $row['max_id'] + 1;


  $sql = "INSERT INTO Tutors (firstname, lastname, age, gender, email, password, country, city) VALUES ('$firstname', '$lastname', '$age', '$gender', '$email', '$password', '$country', '$city')"; //age
  $sql2 = "INSERT INTO users (email, password, role, tutor_id) VALUES ('$email', '$password', 'tutor', $tutor_id)";
  if (($conn->query($sql) && $conn->query($sql2) )  === TRUE) {
    ...
    $_SESSION['id'] = $tutor_id; /////// was $_SESSION['id'] = $id;
...
<html lang="en">
  ...
  <body>
    ...
    <div class="container">
      ...
      <form form method="post" action="Signup1.php">
